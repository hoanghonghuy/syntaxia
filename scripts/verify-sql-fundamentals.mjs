...
const expected = [
  'what-is-sql','sql-syntax','select-queries','select-distinct','filtering-with-where','and-or-not','order-by','limit-rows',
  'null-values','inserting-rows','updating-rows','deleting-rows','min-and-max','count-rows','sum-and-avg','like-pattern','in-list','between-range','column-aliases',
  'inner-join','left-join','right-join','full-join','self-join','union-queries','group-by-aggregate','having-filter','exists-subquery','case-expression',
  'creating-tables','alter-table','drop-table','primary-key','foreign-key','create-index','create-view','sql-wildcards','union-all','insert-into-select','any-all-subquery','table-constraints','sql-comments',
]

// Expand only after a coherent block has been rewritten in both locales.
const v2Migrated = expected.slice(0, 29)
...
function parse(md) {
  ...
  return {
    ...
    order: Number(get('order')), slug: get('slug').replace(/['"]/g, ''), canDo: get('can_do').replace(/^['"]|['"]$/g, ''),
    hasHints: /hints:\s*\n\s*-/.test(fm), hasSol: /solution:/.test(fm), hasPrev: /preview:/.test(fm), hasExp: /expected:/.test(fm), hasSeed: /sandbox_seed:/.test(fm),
    allowsMutations: /allow_mutations:\s*true/.test(fm), hasVerifySql: /verify_sql:\s*["']?SELECT\b/i.test(fm),
    leadingH1: /^\s*#\s+[^#]/m.test(body), table: /\|\s*-+/.test(body), mistakes: /Common mistakes|Lỗi thường gặp/i.test(body), published: get('published'), body,
  }
}

function hasHeading(body, en, vi) { return new RegExp(`^##\\s+(?:${en}|${vi})\\s*$`, 'im').test(body) }
const issues = []
...
for (const loc of ['en', 'vi']) {
  ...
  for (let i = 0; i < expected.length; i++) {
    const slug = expected[i], p = bySlug[slug]
...
const enSlugs = fs.readdirSync(path.join(root,'en')).map(f=>f.replace(/\.md$/,'')).sort().join(',')
const viSlugs = fs.readdirSync(path.join(root,'vi')).map(f=>f.replace(/\.md$/,'')).sort().join(',')
if (enSlugs !== viSlugs) issues.push('en/vi slug mismatch')
for (const slug of expected) {
  const en=parsedByLocale.en[slug], vi=parsedByLocale.vi[slug]
  if (!en || !vi) continue
