...
async function main() {
  const email = `verify_sql_${Date.now()}@test.local`
  const reg = await fetch(`${base}/api/v1/auth/register`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ email, password: 'password123', displayName: 'Verify' }),
  })
  if (!reg.ok) throw new Error(`register ${reg.status} ${await reg.text()}`)
  const cookie = cookieFrom(reg)
  if (!cookie) throw new Error('no auth cookie')

  const listRes = await fetch(`${base}/api/v1/lessons?track=sql-fundamentals&locale=en`, {
    headers: { cookie },
  })
  ...
  for (const sum of lessons.sort((a, b) => a.sortOrder - b.sortOrder)) {
    ...
    if (!run.ok || !body.passed) {
      ...
      console.log('FAIL', sum.slug, body.message || body)
    } else {
      console.log('OK  ', sum.slug)
    }
  }
  if (fail.length) {
  ...
  console.log('PASS all', lessons.length, 'solutions')
}
