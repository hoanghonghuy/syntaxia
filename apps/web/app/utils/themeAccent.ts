 ...
 * Brand hex matches the chosen swatch in both light and dark (no wash-out).
 * Soft / deep / onBrand adapt for surface contrast.
 */
export function deriveAccentTokens(hex: string, appearance: ResolvedAppearance): AccentTokens {
  ...
  const onBrand = lum > 0.5 ? '#0f1419' : '#ffffff'

  if (appearance === 'light') {
    return {
      brand,
      deep: mix(rgb, black, 0.22),
      soft: mix(rgb, white, 0.82),
      onBrand,
    }
  }
  ...
  return {
    brand,
    deep: mix(rgb, white, 0.28),
    soft: mix(rgb, black, 0.82),
    onBrand,
  }
}
