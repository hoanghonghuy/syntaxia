...
Define how **lesson progress** (`lesson_progress.completed`) stays in sync when a learner passes a SQL, JavaScript, or HTML/CSS sandbox exercise — so Progress hub, Continue CTAs, and track % reflect real practice, not only manual Mark complete.

## When to use

- Changing `JsSandbox.vue`, `SqlSandbox.vue`, `HtmlCssSandbox.vue`, or lesson complete UX
- Debugging “passed but not showing on /progress”
- Adding a new graded exercise type

## Steps

1. **Grade pass** — `JsSandbox` / `SqlSandbox` / `HtmlCssSandbox` emit `passed` when server grade returns `passed: true` (logged-in users only).
...
5. **Tests** — `npm run test:learning-path` for helpers; `release-smoke.ps1` for JS/SQL/HTML-CSS grade gates.

## Do / Don't

**Do**

- Keep SQL, JS, and HTML/CSS sandbox parity for progress writes.
...
- [`html-css-sandbox.md`](./html-css-sandbox.md)
